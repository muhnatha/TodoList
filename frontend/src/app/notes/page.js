...
export default function NotesPage() {
  ...
  const [currentNote, setCurrentNote] = useState({ id: null, title: "", content: "", date: "" });
  ...
  const handleCreate = () => {
    setEditing(true);
    setCurrentNote({ id: null, title: "", content: "", date: new Date().toISOString().split("T")[0] });
  };
  ...
  const handleNewNote = () => { // Same as handleCreate
    setEditing(true);
    setCurrentNote({ id: null, title: "", content: "", date: new Date().toISOString().split("T")[0] });
  };
  ...
  return (
    <PageLayout title="NOTES">
      <div className="flex-1 flex flex-col p-4 md:p-8 relative"> {/* Adjusted padding */}
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
          <h1 className="text-2xl md:text-3xl font-bold text-[#1D1C3B] dark:text-slate-100">My Notes</h1>
        ...
        {!user && !isLoading && (
          <div className="text-center py-10">
            <p className="text-lg text-gray-600 dark:text-gray-400">Please log in to manage your notes.</p>
        ...
        {user && !editing && notes.length === 0 && !isLoading && (
          <>
            ...
            <div className="text-gray-400 dark:text-gray-500 text-center mt-32 text-lg">No notes yet...</div>
          </>
        )}
          ...
          <div className="border dark:border-slate-700 p-6 rounded-md shadow-md bg-white dark:bg-slate-800 w-full max-w-3xl mx-auto relative">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-2">
                <button onClick={handleSave} title="Save Note" className="text-slate-600 dark:text-slate-300 hover:text-[#6B5CFF] dark:hover:text-[#8A7FFF]"> <ArrowDownToLine /></button>
                <button onClick={handleClose} title="Close Editor" className="text-slate-600 dark:text-slate-300 hover:text-red-500 dark:hover:text-red-400"><MessageCircleX /></button>
              </div>
            </div>
            <input
              type="text"
        ...
        {isLoading && user && ( // Show loading spinner only if user is logged in and loading notes
            <div className="text-center py-10 text-gray-500 dark:text-gray-400">Loading notes...</div>
        )}

        {user && !editing && notes.length > 0 && !isLoading && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
            {notes.map((note) => ( // Now we pass the full note object to handleEdit
              <div key={note.id} className="border dark:border-slate-700 p-4 rounded-md shadow-md bg-white dark:bg-slate-800 relative">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-gray-500 dark:text-gray-400">{note.date ? new Date(note.date).toLocaleDateString() : 'No Date'}</span>
                  <div className="flex gap-2">
                    <button onClick={() => handleEdit(note)} title="Edit Note" className="text-slate-600 dark:text-slate-300 hover:text-[#6B5CFF] dark:hover:text-[#8A7FFF]"><Pencil size={18}/></button>
                    <button onClick={() => handleDelete(note.id)} title="Delete Note" className="text-slate-600 dark:text-slate-300 hover:text-red-500 dark:hover:text-red-400"><Trash2 size={18}/></button>
                  </div>
                </div>
                <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 break-words">{note.title || "Untitled"}</h2>
                <p className="text-sm mt-2 text-gray-700 dark:text-gray-300 break-words whitespace-pre-wrap">{note.content}</p>
              </div>
            ))}
            ...
            >
             <PlusCircle size={24} className="mb-2"/> {/* Example Icon, import if needed */}
              New Note
            </button>
          </div>
        )}
        ...
        {showConfirmDelete && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"> {/* Added z-index and background */}
            <div className="bg-white dark:bg-slate-800 p-6 rounded-md shadow-lg text-center w-full max-w-sm m-4"> {/* Responsive width */}
              <p className="text-lg font-semibold mb-4 text-slate-900 dark:text-slate-100">Are you sure you want to delete this note?</p>
...
// Make sure to import PlusCircle from lucide-react if you use it in the "New Note" button.
